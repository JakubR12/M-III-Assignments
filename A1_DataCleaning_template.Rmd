...
https://stackoverflow.com/questions/26371279/dplyr-0-3-0-2-rename-idiom-unstable-when-reshape-package-is-loaded
...
#pacman$method3_id <- anonymise(pacman, cols_to_anon) formula
final_df$SUBJ<- anonymise(final_df, cols_to_anon = "SUBJ")


#re-naming levels of gender
final_df$Gender<-ifelse(final_df$Gender==1, "M", "F")

#renaming diagnosis
final_df$Diagnosis <- ifelse(final_df$Diagnosis == "A", "ASD", "TD")

#making csv file
write.csv(final_df, file = "clean_data.csv")
```
...
filter(final_df, cell_type %in% c("NA"))
...
#no change at all when typing the same columns two times
```
...
##creating means of each visit across all participants
...
#3 The advantage of keeping all data is that we can trace progress of each child, its baseline and overall improvent within 6 visits. It gives us much a more accurate picture.
