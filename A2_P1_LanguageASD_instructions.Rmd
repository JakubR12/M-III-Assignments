...
library(pacman)
p_load(dplyr, tidyverse, ggplot2,lme4)
```
...
df %>% subset (VISIT == 1) %>% subset(Gender ==1)# 61 participant, 51 males, 10 females in total

#Autism
subset(df_info, Diagnosis =="A")# 29 with autism
subset(df_info, Diagnosis =="A"& Gender =="M") #25 M, 4F
subset(df_info, Diagnosis =="A") %>% mean(Age, na.rm=TRUE)
...
```{r ex2, include = FALSE}
ggplot(data = df, aes(x = VISIT, y = CHI_MLU, group=Diagnosis)) + geom_point() + geom_smooth(method = lm)
